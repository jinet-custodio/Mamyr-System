...
if (isset($_POST['edit'])) {
    ...
    if (!empty($_FILES['userProfile']['tmp_name']) && $_FILES['userProfile']['error'] === 0) {
        ...
        if ($userProfile_size > 2000000) {
            header("Location: ../Pages/Admin/account.php?message[]=Image%20is%20too%20large%20(max%202MB)");
            exit(0);
        }
    ...
    if ($result) {
        ...
        header("Location: ../Pages/Admin/account.php?id=$userID&role=$userType&message[]=Changes%20Applied!");
        exit(0);
    } else {
